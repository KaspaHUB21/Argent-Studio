...
#[tauri::command] fn app_info(app:tauri::AppHandle)->Result<Value,String>{let root=projects(&app)?;Ok(json!({"resources":resources(&app),"dataDir":data(&app)?,"projectsDir":root,"defaultProject":example_project(root.join("tickets"),"tickets.ag","Tickets"),"compiler":compiler_path(&app,None).ok(),"platform":std::env::consts::OS,"version":app.package_info().version.to_string(),"updatesEnabled":cfg!(any(windows, target_os="macos"))&&!cfg!(debug_assertions)&&!std::env::args().any(|a|a=="--ui-smoke"||a=="--smoke-test"||a=="--verify-update")}))}
#[derive(Serialize)] #[serde(rename_all="camelCase")]
...
(async()=>{
 ...
 try{
  ...
  await app.context.setSettings({...app.state.settings,editorZoom:130});if((await invoke('load_settings')).editorZoom!==130)throw Error('Font preference was not persisted');
  await app.structureWindow.open(current);
  let detached=await app.structureWindow.inspect();if(!detached?.ready||detached.version!==app.state.info.version||(!Number.isFinite(parseFloat(detached.fontSize))||Math.abs(parseFloat(detached.fontSize)-18.2)>.01))throw Error('Detached window or font size mismatch: '+JSON.stringify(detached));
  const originalText=current.text;app.context.updateDocument(current.path,originalText+'\n// detached native synchronization check\n');
  let synchronized=false;for(let i=0;i<50;i++){detached=await app.structureWindow.inspect();if(detached.text===current.text){synchronized=true;break;}await new Promise(r=>setTimeout(r,100));}
  if(!synchronized)throw Error('Detached source did not synchronize');
  if(!await app.structureWindow.dock()||app.structureWindow.isOpen())throw Error('Structure did not dock');
