...
try{
 ...
 try {
  const defaultEnv={...process.env};delete defaultEnv.ARGENT_PROJECTS_DIR;
  const report=path.join(reports,'default-projects.json');run(exe,['--smoke-test','--smoke-report',report],'default-projects',defaultEnv);
  const result=JSON.parse(fs.readFileSync(report,'utf8'));
  if(!result.success||!result.app.projectsDir.endsWith('/Documents/Argent Studio/projects'))throw Error('Default project directory or backend failed');
  const uiReport=path.join(reports,'default-ui.json');run(exe,['--ui-smoke','--ui-smoke-report',uiReport],'default-ui',defaultEnv);
  if(!JSON.parse(fs.readFileSync(uiReport,'utf8')).success)throw Error('Default startup failed');
  summary.defaultProjects=true;
 } catch(e){summary.errors.push('default startup: '+e.message);}
