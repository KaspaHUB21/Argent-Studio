...
test('complete editor workflow uses real compiler, scanner and VM on isolated files',async({page})=>{
 test.setTimeout(120000);const project=path.join(root,'test-output','workflow-'+Date.now());await fs.cp(path.join(resources,'examples/catalog/tickets'),project,{recursive:true});const errors=[];page.on('pageerror',e=>errors.push(e.message));let buildResult;
